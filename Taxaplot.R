...
sample <- "piglet"
plotname=paste("taxaplot",rank,"png",sep=".")
...
N <- length(colours)
temp <- aggregate(ps@otu_table, by=list(as.vector(ps@tax_table[,rank])),FUN=sum)
...
df$Taxa <- factor(df$Taxa,levels=rev(levels(df$Taxa)))
  for (Otu in levels(df$Taxa)){
    if (length(grep("Otu",Otu))!=0){
      levels(df$Taxa)[levels(df$Taxa)==Otu] <- paste(Otu,ps@tax_table[Otu,6])
}}
