...
source("/Users/mdavi/Documents/R_functions/calculate_adonis2.R")
...
ps.rare.Colon <- prune_samples(ps@sam_data$Sample.Type=="Feaces", ps)
ps.rare.Ceacum <- prune_samples(ps@sam_data$Sample.Type=="Ceacum", ps)
ps.rare.Colon.Genus <- tax_glom(ps.rare.Colon, taxrank = "Rank6")
ps.rare.Ceacum.Genus <- tax_glom(ps.rare.Ceacum, taxrank = "Rank6")

#general alpha div plot
png("adiv.facets.png" ,width = 1200, height = 900)
p = plot_richness(ps.rare, x="Treat", color="Treat") + facet_wrap(Sample.Type ~ variable, scale="free")
p = p + geom_boxplot(data = p$data, aes(x = eval(p$mapping$x), y = value, color = NULL), alpha = 0.1)
plot(p)
dev.off()

#general bdiv plot
png("bdiv.pcoa.png" ,width = 1200, height = 900)
p <- plot_ordination(physeq = ps.rare, ordination = ordinate(physeq = ps.rare, method = "PCoA", distance = "bray"), color="Treat", shape="Sample.Type", label="piglet")
p <- p + geom_line(aes(group=animal.number))
p + labs(title="PCoA-bray")
dev.off()

#### test and plot co-variables  OTU level ####
calculate_adonis_bray(ps.rare.Colon)

png("bdiv.colon.pcoa.reladgtot.png" ,width = 1200, height = 900)
p <- plot_ordination(physeq = ps.rare.Colon, ordination = ordinate(physeq = ps.rare.Colon, method = "PCoA", distance = "bray"), color="reladgtot_inv", shape="Treat", label="piglet")
p + labs(title="PCoA-bray-Colon") + scale_color_gradient(low="red", high="green") + geom_point(size=5)
dev.off()

png("bdiv.colon.pcoa.AAcol.png" ,width = 1200, height = 900)
p <- plot_ordination(physeq = ps.rare.Colon, ordination = ordinate(physeq = ps.rare.Colon, method = "PCoA", distance = "bray"), color="AAcol", shape="Treat", label="piglet")
p + labs(title="PCoA-bray-Colon") + scale_color_gradient(low="red", high="green") + geom_point(size=5)
dev.off()

calculate_adonis_bray(ps.rare.Ceacum)

png("bdiv.Ceacum.pcoa.reladgtot.png" ,width = 1200, height = 900)
p <- plot_ordination(physeq = ps.rare.Ceacum, ordination = ordinate(physeq = ps.rare.Ceacum, method = "PCoA", distance = "bray"), color="reladgtot_inv", shape="Treat", label="piglet")
p + labs(title="PCoA-bray-Ceacum") + scale_color_gradient(low="red", high="green") + geom_point(size=5)
dev.off()

png("bdiv.Ceacum.pcoa.AAcol.png" ,width = 1200, height = 900)
p <- plot_ordination(physeq = ps.rare.Colon, ordination = ordinate(physeq = ps.rare.Colon, method = "PCoA", distance = "bray"), color="AAcol", shape="Treat", label="piglet")
p + labs(title="PCoA-bray-Ceacum") + scale_color_gradient(low="red", high="green") + geom_point(size=5)
dev.off()

#### test and plot co-variables Genus level ####

calculate_adonis_bray(ps.rare.Colon.Genus)
png("bdiv.genus.colon.pcoa.VA.png" ,width = 1200, height = 900)
p <- plot_ordination(physeq = ps.rare.Colon.Genus, ordination = ordinate(physeq = ps.rare.Colon.Genus, method = "PCoA", distance = "bray"), color="VA", shape="Treat", label="piglet")
p + labs(title="PCoA-bray-Colon") + scale_color_gradient(low="red", high="green") + geom_point(size=5)
dev.off()

png("bdiv.genus.colon.pcoa.reladgtot_inv.png" ,width = 1200, height = 900)
p <- plot_ordination(physeq = ps.rare.Colon.Genus, ordination = ordinate(physeq = ps.rare.Colon.Genus, method = "PCoA", distance = "bray"), color="reladgtot_inv", shape="Treat", label="piglet")
p + labs(title="PCoA-bray-Colon") + scale_color_gradient(low="red", high="green") + geom_point(size=5)
dev.off()

calculate_adonis_bray(ps.rare.Ceacum.Genus)

#### RDA ####

ord <- ordinate(physeq = ps.rare.Colon, formula = . ~ CELL + AX, method = "RDA", distance = "bray")

p <- plot_ordination(physeq = ps.rare, ordination = ord, color="Treat", shape="CELL", label="Treat")
p + geom_point(size=5) + scale_color_gradient(low="red", high="green")
...
p <- plot_ordination(physeq = ps, ordination = ordinate(physeq = ps, method = "PCoA", distance = "bray"), color="reladgtot_inv", shape="Sample.Type", label="Treat")
p
